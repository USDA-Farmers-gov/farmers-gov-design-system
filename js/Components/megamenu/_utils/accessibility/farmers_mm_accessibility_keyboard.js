...
const topLevelLinks = document.querySelectorAll(".tbm-item.level-1 a");

const firstLevel = document.querySelectorAll(".tbm .tbm-item.level-1");
...
export function homeEnd(event) {
	...
	const level2Selector =
		event.keyCode === 36
			? ".tbm-nav .tbm-item.level-1.open .level-2:first-of-type"
			: ".tbm-nav .tbm-item.level-1.open .level-2:last-child";
	...
	if (
		document.querySelector(`.tbm ${firstLevelSelector}`) &&
		!openLevel2Item
	) {
...
// 	const focusedLink = document.querySelector(".tbm .level-1 a:focus");
...
// 					`li[data-label="${linkTitle.textContent}"]`,
// 				),
// 			);
// 			const newIndex = direction === "right" ? index + 1 : index - 1;

// 			if (firstLevelArray[newIndex]) {
// 				document.querySelector(".tbm .level-1 a:focus").blur();
...
// 		document.querySelectorAll(".tbm .level-1.open li.level-2"),
// 	);
// 	const focusedLink = document.querySelector(
// 		".tbm .level-1.open li.level-2 a:focus",
// 	);

// 	const currentLink = focusedLink
// 		? focusedLink
// 		: document.querySelector(
// 				".tbm .level-1.open li.level-2 a.sidemenu-hover",
// 		  );

// 	const secondLevelElement = currentLink
// 		? document.querySelector(
// 				`.tbm .level-1.open li.level-2[data-label="${currentLink.getAttribute(
// 					"data-category",
// 				)}"]`,
// 		  )
// 		: null;
